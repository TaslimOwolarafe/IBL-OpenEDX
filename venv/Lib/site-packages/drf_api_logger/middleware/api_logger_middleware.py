...
from drf_api_logger.utils import get_headers, get_client_ip, mask_sensitive_data, b_to_dict
...
class APILoggerMiddleware:
    ...
    def __call__(self, request):
        ...
        if self.DRF_API_LOGGER_DATABASE or self.DRF_API_LOGGER_SIGNAL:
            ...
            request_data = ''
            try:
                request_data = b_to_dict(request.body) if request.body else ''
            except:
                pass
            
            request_d = ''
            try:
                request_d = dict(request.data) if request.data else ''
            except:
                pass
            ...
            if response.get('content-type') in ('application/json', 'application/vnd.api+json', 'application/gzip'):
                ...
                data = dict(
                    ...
                    data=mask_sensitive_data(request_d),
                    method=method,
                ...
                if self.DRF_API_LOGGER_DATABASE:
                    if LOGGER_THREAD:
                        ...
                        if request_d:
                            d['data'] = json.dumps(d['data'], indent=4, ensure_ascii=False)
